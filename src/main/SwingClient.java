...
public class SwingClient {
  ...
  public void createAndShowGUI() {
    ...
    // Added new button
    JButton remove_publisher_button = new JButton("remove publisher");
  ...
  // Restore che client profile
  // All the resto of the code is the SAME !!!
  private void clientSetup() {
      
    String login = ((TopicManagerStub) topicManager).user.getLogin(); 
    
    messages_TextArea.append("LOGIN: Login of user: "+login+"\n");
    //this is where you restore the user profile:
    SwingClient.this.publisher = topicManager.publisherOf();
    if (SwingClient.this.publisher != null) {
        //retrive the publuisher topic
        SwingClient.this.publisherTopic = SwingClient.this.publisher.topic();
        System.out.println("USER: "+login+" is publisher of "+publisherTopic);
        publisher_TextArea.setText(SwingClient.this.publisher.topic().name);
    }
    else {
        System.out.println("USER: "+login+" is NOT publisher");
    }
    
    // Retrive subsciptions
    
    List<entity.Subscriber> list_subscriptions = ((TopicManagerStub) topicManager).mySubscriptions();
    
    my_subscriptions_TextArea.setText("");
    
    for (entity.Subscriber sub : list_subscriptions){
        Subscriber subscriber = new SubscriberImpl(SwingClient.this);
        SwingClient.this.my_subscriptions.put(sub.getTopic(), subscriber);
        my_subscriptions_TextArea.append(sub.getTopic().getName()+"\n");
        // Subscribe to the topic
        Subscription_check chk_topic = topicManager.subscribe(sub.getTopic(), subscriber);
        List<Message> list_messages = topicManager.messagesFrom(sub.getTopic());
